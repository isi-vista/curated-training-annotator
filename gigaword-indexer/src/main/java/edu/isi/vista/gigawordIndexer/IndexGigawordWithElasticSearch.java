...
/**
 * Indexes Gigaword with Elastic Search in a way usable by the external search feature of the
 * Inception annotator.
 *
 * <p>See usage message for details.
 */
public class IndexGigawordWithElasticSearch {
  private static final String USAGE =
      "IndexGigawordWithElasticSearch param_file\n"
  ...
  public static void main(String[] argv) throws IOException {
    ...
    try (RestHighLevelClient client = buildElasticSearchClient(parameters)) {
      ...
      if (parameters.isPresent(PARAM_GIGAWORD_DIRECTORY_PATH)) {
        final PathMatcher gzippedConcatenatedFilePattern =
            FileSystems.getDefault().getPathMatcher("glob:**/data/**/*.gz");
  ...
  private static void index(RestHighLevelClient client, Path file, String indexName)
      throws IOException {
    log.info("Indexing {}", file.toAbsolutePath());

    // we batch the documents in groups of 100 so we can get the efficiency gains from batching
    // without
    // making huge requests of unbounded size
    for (List<Article> articles :
        partition(ConcatenatedGigawordDocuments.fromGigwordGZippedFile(file), 100)) {
